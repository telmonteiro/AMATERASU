...
class gls_periodogram:
    ...
    def __init__(self, star, period_test, ind, x, y, y_err=None, pmin=1.5, pmax=1e4, steps=1e5, verb=True, save=False, folder_path=None):
        self.star = star
    ...
    def compute_periodogram(self):
        ...
        sel_peaks_dict = self._get_sign_gls_peaks(gls, dict_peaks, dict_peaks_WF, gaps, fap5, atol_frac=0.1, verb=False, evaluate_gaps=False)
        
        self.results.update({'gaps': gaps,'sel_peaks_period': sel_peaks_dict["sel_peaks_period"],
                             'sel_peaks_power': sel_peaks_dict["sel_peaks_power"], "sel_peaks_FAP": sel_peaks_dict["sel_peaks_FAP"]})
        ...
        self.results.update({'period_best': round(period_best,3), 'power_best': power_best, 'fap_best': fap_best, 'flag':flag})
    ...
    def plot_periodogram(self, period, power, sel_peaks, fap5, fap1, fap01, save=False):
        ...
        axes.axvspan(self.period_test[0] - self.period_test[1], self.period_test[0] + self.period_test[1], alpha=0.6, color='orange', label="Test Period")
        ...
        axes.text(0.13, 0.6, f"P = {np.around(self.results['period_best'],1)} d ({self.results['flag']})", fontsize=14, transform=plt.gcf().transFigure)
    ...
    def _get_sign_gls_peaks(self, gls, dict_peaks, dict_peaks_WF, gaps, fap5, atol_frac=0.05, verb=False, evaluate_gaps=False):
    ...
    def _periodogram_flagging(self, fap_best):
        ...
        if fap_best >= 0.05:
            ...
            if 0.01 <= fap_best < 0.05:
                flag = 1
            elif 0.001 <= fap_best < 0.01:
                flag = 2
            else:
                flag = 3
