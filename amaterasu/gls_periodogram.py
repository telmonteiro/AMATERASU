...
class gls_periodogram:
    ...
    def __init__(self, star, period_test, ind, x, y, y_err, pmin=1.5, pmax=1e3, steps=1e4, folder_path=None):
        ...
        results.update({'freq': list(freq),'period': list(period),'power': list(power),'fap_maxp': fap_max_power,'fap01_power': fap01_power,'fap1_power': fap1_power,'FAPS': list(faps)})
        ...
        results.update({'gaps': list(gaps),'sel_peaks_period': list(sel_peaks_dict["sel_peaks_period"]),'sel_peaks_power': list(sel_peaks_dict["sel_peaks_power"]), "sel_peaks_FAP": list(sel_peaks_dict["sel_peaks_FAP"])})
        ...
        if folder_path:
            self.plot_periodogram(ind, star, period_test, results, results["period"], results["power"], results["sel_peaks_period"], results['fap1_power'], results['fap01_power'], folder_path)
            self.plot_window_function(ind, star, results, results["period"], results["power_win"], results['fap1_power'], results['fap01_power'], folder_path)

        gls_periodogram.results = results


    def _get_sign_gls_peaks(self, gls, dict_peaks, dict_peaks_WF, atol_frac=0.05):
        ...
        sign_peaks_win = [per for per, fap in zip(dict_peaks_WF['peaks_period_win'], sign_peaks_fap) if fap < 0.001]
    ...
    def GLS_plot(star, ind, p_rot_lit_list, bandpasses, df_clean, all_gls_df, folder):
        ...
        for i,bp in enumerate(bandpasses):
            ...
            axes[i,1].plot([min(results["period"]), max(results["period"])], [results["fap01_power"]] * 2,"--",color="black",lw=0.7)
            axes[i,1].plot([min(results["period"]), max(results["period"])], [results["fap1_power"]] * 2,"--",color="black",lw=0.7)
